...
def train(args):
    ...
    print(f"Model: LMSR with Wavelet-Enhanced Latent Space")
    ...
    for epoch in range(args.epochs):
        ...
        for i, (lr_img, hr_img, _, _) in enumerate(dl):
            ...
            with torch.no_grad():
                z_lr = model.encode(lr_img)
                z_hr = model.encode(hr_img)
            ...
            if i % 5 == 0:
                with torch.no_grad():
                    ...
                    pred_img = model.decode(x_start, use_freq_bridge=True)
            ...
            for k in running_losses.keys():
                running_losses[k] += losses.get(k, torch.tensor(0.0)).item()
