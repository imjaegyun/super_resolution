# From https://github.com/HVision-NKU/SRFormer
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224,window_size =22,patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        if (img_size[0] % window_size != 0) or (img_size[1] % window_size != 0):  # FIXED
            img_size = (img_size[0] + window_size - img_size[0] % window_size,
                        img_size[1] + window_size - img_size[1] % window_size)
...
class PatchUnEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, window_size =24,patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        if (img_size[0] % window_size != 0) or (img_size[1] % window_size != 0):
            img_size = (img_size[0] + window_size - img_size[0] % window_size,
                        img_size[1] + window_size - img_size[1] % window_size)
        patches_resolution = [img_size[0] // patch_size[0], img_size[1] // patch_size[1]]
        self.img_size = img_size
...
if __name__== '__main__':
    ...
    upsampling_factor = 3
    window_size = 16
    ds_height, ds_width = height // upsampling_factor, width // upsampling_factor
    ...
    # x = torch.FloatTensor(batch_size, 3, _height, _width).uniform_(0, 1).to(torch.float32)
    # shape = (batch_size, 3, _height, _width)
    shape = (batch_size, 3, height//upsampling_factor, width//upsampling_factor)
    model = SRFormer(**model_kwargs)
